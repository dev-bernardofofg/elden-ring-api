...
interface CreaturesDialogProps {
  children: ReactNode;
  data: any;
}

export const CreaturesDialog = ({ children, data }: CreaturesDialogProps) => {
  const [datas, setData] = useState<CreaturesProps | null>(null);
  const { toast } = useToast();
  const getData = async () => {
    try {
      const response = await axios.get(
        `https://eldenring.fanapis.com/api/creatures/${data}`
      );
