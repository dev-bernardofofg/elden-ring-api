...
export const ShieldDialog = ({ children, data }: ShieldDialogProps) => {
  const [datas, setData] = useState<ShieldProps | null>(null);
  const { toast } = useToast();
  const getData = async () => {
    try {
      const response = await axios.get(
        `https://eldenring.fanapis.com/api/shields/${data}`
      );
