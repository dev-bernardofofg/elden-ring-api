...
interface TalismanDialogProps {
  children: ReactNode;
  data: any;
}

export const TalismanDialog = ({ children, data }: TalismanDialogProps) => {
  const [datas, setData] = useState<TalismanProps | null>(null);
  const { toast } = useToast();
  const getData = async () => {
    try {
      const response = await axios.get(
        `https://eldenring.fanapis.com/api/talismans/${data}`
      );
